...
<div class="wrap">
    <div id="poststuff">
        <div id="post-body" class="metabox-holder columns-2">
            ...
            <?php
			if (isset($_POST['bf_xprofile_conditional_visibility']) && is_array(wc4bp_clean_input(wp_unslash($_POST['bf_xprofile_conditional_visibility'])))) {
				foreach (wc4bp_clean_input(wp_unslash($_POST['bf_xprofile_conditional_visibility'])) as $group_id => $group_options) {
					...
					if (array_key_exists('products', $group_options)) {
						...
						if ($deleteProd) {
							...
							$existing_product_ids = bp_xprofile_get_meta(
								$group_id,
								'group',
							...
							if ($existing_product_ids != $updated_product_ids) {
								if (bp_xprofile_update_fieldgroup_meta(
									$group_id,
									'bf_xprofile_conditional_visibility_products',
									$updated_product_ids
								) === false) {
					...
					if (array_key_exists('categories', $group_options)) {
						...
						if ($deleteCategory) {
							...
							$existing_category_ids = bp_xprofile_get_meta(
								$group_id,
								'group',
			...
			if (isset($_POST['bf_xprofile_options'])) {
				update_option('bf_xprofile_options', wc4bp_clean_input(wp_unslash($_POST['bf_xprofile_options'])));
			}

			if (isset($_POST['wc4bp_sync_mail'])) {
				update_option('wc4bp_sync_mail', wc4bp_clean_input(wp_unslash($_POST['wc4bp_sync_mail'])));
			} else {
...
<div id="tabs">
    <ul id="field-group-tabs" class="nav tabs" style="display: block;">
		...
		if (!empty($groups)):
			foreach ($groups as $group):
				?>
        <li id="group_<?php echo esc_attr($group->id); ?>"><a href="#tabs-<?php echo esc_attr($group->id); ?>"
                class="ui-tab"><?php echo esc_attr($group->name); ?>
                <?php
				if (!$group->can_delete):
					?>
    ...
    <div id="tabs-<?php echo esc_attr($group->id); ?>" class="tab-wrapper">
        ...
        <fieldset id="<?php echo esc_attr($group->id); ?>" class="field-group">
...
<fieldset id="field_<?php echo esc_attr($field->id); ?>" class="sortable
								   <?php
	...
	">
    <legend>
        <span><b><?php bp_the_profile_field_name(); ?> </b>
            <?php
			if (!$field->can_delete):
				?>
    ...
    <div class="field-wrapper">
        ...
        <input
            <?php isset($bf_xprofile_options[esc_attr($admin_group->id)][esc_attr($field->id)]['hide']) ? checked('hide', $bf_xprofile_options[esc_attr($admin_group->id)][esc_attr($field->id)]['hide']) : ''; ?>
            type="checkbox"
        ...
        Add to Checkout:
        <input
            <?php isset($bf_xprofile_options[esc_attr($admin_group->id)][esc_attr($field->id)]['checkout']) ? checked('checkout', $bf_xprofile_options[esc_attr($admin_group->id)][esc_attr($field->id)]['checkout']) : ''; ?>
            type="checkbox"
            name="bf_xprofile_options[<?php echo esc_attr($admin_group->id); ?>][<?php echo esc_attr($field->id); ?>][checkout]"
            value="checkout">
        Add to order emails:
        <input
            <?php isset($bf_xprofile_options[esc_attr($admin_group->id)][esc_attr($field->id)]['email']) ? checked('email', $bf_xprofile_options[esc_attr($admin_group->id)][esc_attr($field->id)]['email']) : ''; ?>
            type="checkbox"
            name="bf_xprofile_options[<?php echo esc_attr($admin_group->id); ?>][<?php echo esc_attr($field->id); ?>][email]"
            value="email">
        Display field value on the order edit page:
        <input
            <?php isset($bf_xprofile_options[esc_attr($admin_group->id)][esc_attr($field->id)]['order_edit']) ? checked('order_edit', $bf_xprofile_options[esc_attr($admin_group->id)][esc_attr($field->id)]['order_edit']) : ''; ?>
            type="checkbox"
        ...
        <a target="_blank"
            href="?page=bp-profile-setup&group_id=<?php echo esc_attr($admin_group->id); ?>&field_id=<?php echo esc_attr($field->id); ?>&mode=edit_field">Edit
            this field</a>
        </p>
    </div>
</fieldset>
...
function wc4bp_admin_enqueue_scripts()
{
	...
	wp_enqueue_style(
		'admin-xprofile.css',
		WC4BP_xProfile::plugin_base_url() . 'assets/css/admin-xprofile.css',
		array(
			'select2',
...
function wc4bp_clean_input($input)
{
	if (is_array($input)) {
